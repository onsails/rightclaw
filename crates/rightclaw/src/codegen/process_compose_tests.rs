...
// ── cloudflared script path ──────────────────────────────────────────────────

#[test]
fn cloudflared_entry_absent_when_script_path_none() {
    let agents = vec![make_bot_agent("myagent", "123:tok")];
    let exe = Path::new(EXE_PATH);
    let output = generate_process_compose(&agents, exe, false, None).unwrap();
    assert!(
        !output.contains("cloudflared:"),
        "cloudflared entry must be absent when script_path is None:\n{output}"
    );
}

#[test]
fn cloudflared_entry_present_when_script_path_some() {
    let agents = vec![make_bot_agent("myagent", "123:tok")];
    let exe = Path::new(EXE_PATH);
    let script = "/home/user/.rightclaw/scripts/cloudflared-start.sh";
    let output = generate_process_compose(&agents, exe, false, Some(script)).unwrap();
    assert!(
        output.contains("cloudflared:"),
        "cloudflared entry must be present when script_path is Some:\n{output}"
    );
}

#[test]
fn cloudflared_command_is_script_path() {
    let agents = vec![make_bot_agent("myagent", "123:tok")];
    let exe = Path::new(EXE_PATH);
    let script = "/home/user/.rightclaw/scripts/cloudflared-start.sh";
    let output = generate_process_compose(&agents, exe, false, Some(script)).unwrap();
    let expected = format!("command: \"{script}\"");
    assert!(
        output.contains(&expected),
        "cloudflared command must be the script path:\n{output}"
    );
}

#[test]
fn cloudflared_command_does_not_contain_tunnel_run_token() {
    let agents = vec![make_bot_agent("myagent", "123:tok")];
    let exe = Path::new(EXE_PATH);
    let script = "/home/user/.rightclaw/scripts/cloudflared-start.sh";
    let output = generate_process_compose(&agents, exe, false, Some(script)).unwrap();
    assert!(
        !output.contains("tunnel run --token"),
        "cloudflared command must not contain inline 'tunnel run --token':\n{output}"
    );
}
