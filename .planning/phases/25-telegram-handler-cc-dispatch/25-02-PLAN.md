...
must_haves:
  truths:
    - "split_message splits >4096 chars at last \\n before boundary; hard-cuts if no \\n in last 200 chars"
    - "format_batch_xml produces <messages><msg id='' ts='' from='user'>...</msg></messages> XML"
    - "format_error_reply produces the exact error string: ⚠️ Agent error (exit N):\\n```\\n<stderr 300-char truncated>\\n```"
    - "content:null from reply tool → no send; missing reply tool call → error reply"
    - "worker module exports DebounceMsg, WorkerContext, spawn_worker"
  artifacts:
    - path: "crates/bot/src/telegram/worker.rs"
      provides: "debounce loop, CC subprocess invocation, reply tool parsing, typing indicator"
      exports:
  ...
  key_links:
    - from: "crates/bot/src/telegram/worker.rs"
      ...
      via: "get_session / create_session / touch_session calls"
      pattern: "session::"
    - from: "crates/bot/src/telegram/worker.rs"
      to: "tokio::process::Command"
      via: "CC subprocess invocation with HOME=$AGENT_DIR"
      pattern: "HOME.*agent_dir"
    - from: "crates/bot/src/telegram/worker.rs"
      to: "claude -p --append-system-prompt"
      via: "REPLY_TOOL_JSON const injected on every invocation per D-03"
      pattern: "append.system.prompt"
---

<objective>
Implement the per-session worker task: debounce loop, CC subprocess invocation, reply tool parsing, typing indicator, and text splitting.

Purpose: This is the core dispatch logic. Plan 03 wires it into the teloxide schema — plan 02 must be correct and tested in isolation before that wiring happens.

Output: `worker.rs` with `DebounceMsg`, `WorkerContext`, `spawn_worker`, and all pure/unit-testable helpers fully covered by TDD.
</objective>
...
From 25-01 session.rs (produced by plan 01):
```rust
...
CC subprocess invocation pattern (D-13, D-14, DIS-01, DIS-02):
```rust
tokio::process::Command::new(&cc_bin)
    .arg("-p")
    .arg("--session-id").arg(&uuid)           // first call
    // .arg("--resume").arg(&root_session_id) // resume calls
    .arg("--output-format").arg("json")
    .arg("--system-prompt-file").arg(".claude/system-prompt.txt")  // first call only
    .arg("--").arg(&batched_xml)
    .env("HOME", &agent_dir)
...
CC binary resolution (D-12):
```rust
which::which("claude").or_else(|_| which::which("claude-bun"))
```

Reply tool JSON schema (D-04):
```json
{
  "name": "reply",
  "input_schema": {
...
Error reply format (D-16):
```
⚠️ Agent error (exit N):
```
<stderr truncated to 300 chars>
```
```

split_message algorithm (D-17):
- If `text.len() <= 4096` → return `[text]`
- Split at `last '\n'` in the window `[limit-200..limit]`; if no `'\n'`, hard-cut at `limit`
- Repeat until remaining is empty

Typing indicator strategy (D-10, Pattern 4 from RESEARCH.md):
Use `tokio_util::sync::CancellationToken` — cancel token passed to typing task, `token.cancel()` after subprocess completes.
Send `ChatAction::Typing` every 4 seconds; only set `message_thread_id` when `effective_thread_id != 0`.
</interfaces>

<tasks>

<task type="auto" tdd="true">
  <name>Task 1: Implement pure helpers in worker.rs (TDD — no async, no network)</name>
  <read_first>
  ...
  <files>crates/bot/src/telegram/worker.rs, crates/bot/src/telegram/mod.rs</files>
  <behavior>
    RED phase — write failing tests first for these pure functions:

    `format_batch_xml`:
    - `batch_xml_single_message`: one message → `<messages><msg id="100" ts="..." from="user">hello</msg></messages>`
    - `batch_xml_multi_message`: two messages → two `<msg>` elements in order
    - `batch_xml_escapes_special_chars`: `<`, `>`, `&` in message text → `&lt;`, `&gt;`, `&amp;`

    `split_message`:
    - `split_short_message`: text.len() ≤ 4096 → single-element vec with same text
    - `split_at_newline`: text longer than 4096 with `\n` within last 200 chars → split at that `\n`
    - `split_hard_cut`: text longer than 4096 with no `\n` in last 200 chars → hard-cut at 4096

    `format_error_reply`:
    - `error_reply_with_stderr`: exit code 1, stderr "something failed" → string contains `⚠️ Agent error (exit 1):` and `something failed`
    - `error_reply_truncates_stderr`: stderr > 300 chars → truncated to 300 chars in output

    `parse_reply_tool`:
    - `parse_reply_content_string`: CC JSON with `tool_use` block name="reply", input.content="hello" → `Ok(Some(ReplyOutput { content: Some("hello"..), .. }))`
    - `parse_reply_content_null`: CC JSON with content=null → `Ok(Some(ReplyOutput { content: None, .. }))`
    - `parse_no_tool_call`: CC JSON with no `tool_use` block → `Err` (triggers DIS-06 error reply path)

    Run `cargo test -p rightclaw-bot --lib worker` — must fail (RED).
  </behavior>
  <action>
Create `crates/bot/src/telegram/worker.rs`.
...
pub struct DebounceMsg {
    pub message_id: i32,        // Telegram message_id (for reply_to_message_id)
    pub text: String,
...
pub struct WorkerContext {
    ...
    pub bot: crate::telegram::bot::BotType,  // CacheMe<Throttle<Bot>> alias
    pub db_path: PathBuf,   // agent_dir — passed separately so worker opens its own Connection
}
...
pub struct ReplyOutput {
    ...
    pub media_paths: Option<Vec<String>>,  // STUB: Phase 25 warns and skips
}
```

**Step 2: `format_batch_xml`**
...
**Step 5: `parse_reply_tool`**

CC `--output-format json` with tool_use blocks returns something like:
```json
{
  "session_id": "...",
  "result": [...],  // content array: [{type:"tool_use", name:"reply", input:{...}}]
  ...
}
```
The actual format needs empirical verification (Open Question #1 from RESEARCH.md). Write a flexible parser:
...
struct CcOutput {
    ...
    // result can be string or array — use Value
    result: Option<serde_json::Value>,
    // CC also puts content at top level in some versions
    #[serde(default)]
...
///
/// Returns `Ok(Some(ReplyOutput))` if the reply tool was called.
/// Returns `Err(String)` if no tool call found (triggers error reply per D-05).
/// Returns `Ok(Some(ReplyOutput { content: None, .. }))` if content=null (silent response).
pub fn parse_reply_tool(
    raw_json: &str,
) -> Result<(ReplyOutput, Option<String>), String> {
    ...
    Ok((ReplyOutput { content, reply_to_message_id, media_paths }, session_id))
}
...
**Step 6: Write failing tests first (TDD — place in `#[cfg(test)]` module at bottom of `worker.rs`)**
...
mod tests {
    ...
    fn error_reply_truncates_long_stderr() {
        ...
        // The x-block in the reply should not exceed 300 chars of stderr
        let x_block: String = reply.chars().filter(|&c| c == 'x').collect();
        assert_eq!(x_block.len(), 300);
    }
    ...
    fn parse_reply_content_null() {
        let json = r#"{"result":[{"type":"tool_use","name":"reply","input":{"content":null}}]}"#;
...
Add `pub mod worker;` to `crates/bot/src/telegram/mod.rs`.
Also add `pub type BotType = teloxide::adaptors::CacheMe<teloxide::adaptors::throttle::Throttle<teloxide::Bot>>;` to `mod.rs` for use by WorkerContext.

**REMINDER:** Run RED first (failing tests), then implement GREEN. Follow TDD discipline.
  </action>
  <verify>
    <automated>cd /home/wb/dev/rightclaw && cargo test -p rightclaw-bot --lib worker 2>&1 | tail -20</automated>
  </verify>
  <done>
    All worker unit tests pass (11+ tests). `cargo test -p rightclaw-bot --lib worker` exits 0. `worker.rs` contains `DebounceMsg`, `WorkerContext`, `ReplyOutput`, `format_batch_xml`, `split_message`, `format_error_reply`, `parse_reply_tool`. `mod.rs` has `pub mod worker;` and `BotType` alias.
  </done>
</task>

<task type="auto">
  <name>Task 2: Implement spawn_worker async function (debounce loop + CC invocation + typing indicator)</name>
  <read_first>
  ...
  <files>crates/bot/src/telegram/worker.rs</files>
  <action>
Add `spawn_worker` to `worker.rs`. This is the async core of the dispatch loop — cannot be unit-tested without live infrastructure, so focus on correctness via code review pattern (RESEARCH.md Pitfalls).
...
use teloxide::types::{ChatId, ChatAction, ThreadId};

use super::session::{get_session, create_session, touch_session};

type SessionKey = (i64, i64); // (chat_id, effective_thread_id)

const DEBOUNCE_MS: u64 = 500; // D-01: fixed 500ms window
        ...
        loop {
            ...
            let typing_task = tokio::spawn(async move {
                loop {
                    ...
                    if eff_thread_id != 0 {
                        action = action.message_thread_id(ThreadId(eff_thread_id as i32));
                    }
                ...
                Ok(Some(output)) => {
                    if let Some(content) = output.content {
                        ...
                        for part in parts {
                            ...
                            if eff_thread_id != 0 {
                                send = send.message_thread_id(ThreadId(eff_thread_id as i32));
                            }
                            if let Some(ref_id) = output.reply_to_message_id {
                                send = send.reply_to_message_id(ref_id);
                            }
...
async fn invoke_cc(
    ...
    // D-03: reply tool definition injected on every CC invocation via --append-system-prompt.
    // CC must call the `reply` tool — plain text responses fail parse_reply_tool.
    // --append-system-prompt (inline string) works alongside --system-prompt-file (no conflict).
    ...
    tracing::info!(?chat_id, ?eff_thread_id, is_first_call, "invoking claude -p");
    ...
    match parse_reply_tool(&raw) {
        ...
        Err(reason) => {
            ...
            Err(format!("⚠️ Agent error: {reason}\nRaw output (truncated): {}",
...
Add necessary imports to the top of `worker.rs`. Ensure `chrono` is used for `DateTime<Utc>` — add `chrono` to `crates/bot/Cargo.toml` if not already present (check first: `teloxide` re-exports chrono but may not expose it). If unavailable, use `std::time::SystemTime` with manual formatting.

**Note on chrono:** If `teloxide` does not re-export `chrono` types, add `chrono = { version = "0.4", features = ["serde"] }` to workspace deps and bot Cargo.toml.
  </action>
  <verify>
    <automated>cd /home/wb/dev/rightclaw && cargo build -p rightclaw-bot 2>&1 | tail -20</automated>
  </verify>
  <done>
    `cargo build -p rightclaw-bot` exits 0. `worker.rs` contains `spawn_worker`, `invoke_cc`, all data types. All previously passing unit tests still pass (`cargo test -p rightclaw-bot --lib`).
  </done>
</task>
...
<verification>
```
cd /home/wb/dev/rightclaw && cargo test -p rightclaw-bot --lib 2>&1 | tail -20
```
All tests green. No regressions in session tests from plan 01.
</verification>

<success_criteria>
- All 11+ unit tests in `worker.rs` pass
- `format_batch_xml` escapes `<`, `>`, `&`; wraps in `<messages>` XML
- `split_message` splits at `\n` boundary; hard-cuts at 4096 if no `\n` in last 200
- `format_error_reply` truncates stderr at 300 chars; contains `⚠️ Agent error (exit N):`
- `parse_reply_tool` handles `content: null` (silent) and missing tool call (error)
- `spawn_worker` compiles with all required logic: debounce, typing indicator, CC invoke, reply send
- CC subprocess uses `HOME=$AGENT_DIR`, `wait_with_output()`, `stdin(Stdio::null())`, `kill_on_drop(true)`
</success_criteria>

<output>
After completion, create `.planning/phases/25-telegram-handler-cc-dispatch/25-02-SUMMARY.md`
</output>
